...
class download():
    def __init__(self):
        ...
        for i, p in zip(ip, port):
            self.iplist.append(i.strip() + ':' + p.strip())
    ...
    def get_img(self, url, timeout, referer_url, proxy=None, num_retries=6):  ##给函数一个默认参数proxy为空
        ...
        if proxy == None:  ##当代理为空时，不使用代理获取response（别忘了response啥哦！之前说过了！！）
            try:
                ...
                if num_retries > 0:  ##num_retries是我们限定的重试次数
                    ...
                    return self.get(url, timeout, referer_url, num_retries=num_retries - 1)  ##调用自身 并将次数减1
                else:
                    print(u'开始使用代理')
                    time.sleep(3 + random.random() * 3)
                    IP = ''.join(str(random.choice(self.iplist)).strip())  ##下面有解释哦
                    proxy = {'http': IP}
                    return self.get(url, timeout, referer_url, proxy=proxy, )  ##代理不为空的时候
            ...
            try:
                ...
                if num_retries > 0:
                    ...
                    print(u'当前代理是：', proxy)
                    return self.get(url, timeout, referer_url, proxy=proxy, num_retries=num_retries - 1)
                else:
                    print(u'代理也不好使了！取消代理')
                    return self.get(url, 3, referer_url)
    ...
    def get(self, url, timeout, proxy=None, num_retries=6):  ##给函数一个默认参数proxy为空
        ...
        if proxy == None:  ##当代理为空时，不使用代理获取response（别忘了response啥哦！之前说过了！！）
            try:
                ...
                if num_retries > 0:  ##num_retries是我们限定的重试次数
                    time.sleep(9+random.random())  ##延迟十秒
                    print(u'获取网页出错，10S后将获取倒数第：', num_retries, u'次')
                    ...
                    IP = ''.join(str(random.choice(self.iplist)).strip())  ##下面有解释哦
                    proxy = {'http': IP}
                    return self.get(url, timeout, proxy=proxy, )  ##代理不为空的时候
            ...
            try:
                ...
                if num_retries > 0:
                    ...
                    print(u'当前代理是：', proxy)
                    return self.get(url, timeout, proxy=proxy, num_retries=num_retries - 1)
                else:
                    print(u'代理也不好使了！取消代理')
                    return self.get(url, 3)

    def get_all(self, url, timeout):  ##给函数一个默认参数proxy为空
        UA = random.choice(self.user_agent_list)  ##从self.user_agent_list中随机取出一个字符串
        headers = {'User-Agent': UA,
